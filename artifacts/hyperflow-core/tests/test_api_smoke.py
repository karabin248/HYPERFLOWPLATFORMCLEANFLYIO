...
class TestRepositoriesScan:
    @pytest.mark.asyncio
    async def test_repositories_scan_rejects_plain_http(self, transport, base_url):
        async with AsyncClient(transport=transport, base_url=base_url) as client:
        ...
        data = resp.json()
        assert data["repositories"][0]["status"] == "failed"
        assert "Only https:// is permitted" in data["repositories"][0]["error"]

    @pytest.mark.asyncio
    async def test_repositories_scan_rejects_file_scheme(self, transport, base_url):
        async with AsyncClient(transport=transport, base_url=base_url) as client:
            resp = await client.post("/v1/repositories/scan", json={
        ...
        data = resp.json()
        assert data["repositories"][0]["status"] == "failed"
        assert "not allowed" in data["repositories"][0]["error"]
